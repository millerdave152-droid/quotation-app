/**
 * Import Samsung TV products from PG Roadmap Excel
...
async function importSamsungTV() {
  const path = 'C:/Users/WD-PC1/OneDrive/Desktop/price/PG-TV Roadmap 2025-12-15.xlsx';
  const workbook = XLSX.readFile(path);
  const sheet = workbook.Sheets['PG'];
  const data = XLSX.utils.sheet_to_json(sheet, { header: 1 });

  console.log('Importing Samsung TV products...\n');
  ...
  for (const product of productList) {
    ...
    // Build category from Category + Series + Size
    const category = [product.category, product.series, product.size ? `${product.size}"` : '']
      .filter(Boolean).join(' ') || 'TV';

    // Build description
    const description = `Samsung ${product.series || ''} ${product.size ? product.size + '"' : ''} ${product.year || ''} TV`.trim();
  ...
  console.log('\n========================================');
  console.log('Samsung TV Import Complete');
  console.log('========================================');
  ...
  const sample = await pool.query(`
    ...
    FROM products
    WHERE manufacturer = 'SAMSUNG' AND category LIKE '%TV%'
    ORDER BY updated_at DESC
    LIMIT 15
  `);

  console.log('\nSample Samsung TV products:');
