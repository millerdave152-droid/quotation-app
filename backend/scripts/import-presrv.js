/**
 * Import Presrv products with MSRP and cost
...
async function importPresrvProducts() {
  ...
  console.log('Importing Presrv products...\n');
  ...
  for (let i = 4; i < data.length; i++) {
    ...
    const fullCategory = category ? `Presrv - ${category}` : 'Presrv';
      ...
      if (existing.rows.length > 0) {
        ...
        await pool.query(
          `UPDATE products SET
            ...
            updated_at = NOW()
          WHERE id = $8`,
          [msrpCents || 0, costCents || 0, 'PRESRV', fullCategory, description, description, mapCents || 0, existing.rows[0].id]
        );
        totalUpdated++;
      } else {
        // Insert new product
        await pool.query(
          ...
          [model, 'PRESRV', fullCategory, description, description, costCents, msrpCents, mapCents]
        );
        totalImported++;
      }
    } catch (err) {
  ...
  console.log('\n========================================');
  console.log('Presrv Import Complete');
  console.log('========================================');
  ...
  const sample = await pool.query(`
    ...
    FROM products
    WHERE manufacturer = 'PRESRV'
    ORDER BY updated_at DESC
    LIMIT 10
  `);

  console.log('\nSample Presrv products:');
  ...
  const total = await pool.query(`
    SELECT COUNT(*) as count FROM products WHERE manufacturer = 'PRESRV'
  `);
  console.log('\nTotal Presrv products:', total.rows[0].count);

  await pool.end();
}

importPresrvProducts().catch(err => {
