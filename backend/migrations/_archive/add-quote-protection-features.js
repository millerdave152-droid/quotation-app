...
async function addQuoteProtectionFeatures() {
  ...
  try {
    ...
    // 3. Insert default follow-up email templates
    const template1Body = `Hi {customer_first_name},
...
Best,
{sales_rep_name}',
        '["customer_first_name", "quote_number", "product_summary", "quote_expiry_date", "sales_rep_name"]'::jsonb,
        '["Ask if there are any obstacles to moving forward", "Mention competitor comparisons if applicable", "Emphasize time-sensitive rebates/promotions", "Offer to adjust quote if needed", "Provide customer testimonials"]'::jsonb,
        false
      ),
      (
...
Thanks,
{sales_rep_name}',
        '["customer_first_name", "quote_number", "product_summary", "quote_expiry_date", "days_until_expiry", "sales_rep_phone", "sales_rep_name"]'::jsonb,
        '["Create urgency with expiring rebates", "Offer to negotiate if needed", "Address last-minute objections", "Suggest alternative products if budget is tight", "Get commitment or close the quote"]'::jsonb,
        false
      ),
      (
...
Take care,
{sales_rep_name}',
        '["customer_first_name", "quote_number", "sales_rep_name"]'::jsonb,
        '["Give customer permission to say no", "Open door for future business", "Ask for feedback on why they didn''t purchase", "Offer to stay in touch", "Archive quote if no response"]'::jsonb,
        false
      ),
      (
...
{company_name}',
        '["customer_name", "company_name", "quote_number", "quote_total", "quote_expiry_date", "sales_rep_name", "sales_rep_email", "sales_rep_phone"]'::jsonb,
        '[]'::jsonb,
        true
      )
...
`.trim();
    ...
    // 7. Create index for tracking performance
    await client.query(`
    ...
    console.log('  • Email templates (4 follow-up templates + 1 confirmation)');
