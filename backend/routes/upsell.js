  ...
  try {
    ...
    if (!cart || !cart.items) {
      return res.status(400).json({
        success: false,
        error: 'Cart data with items is required',
      });
    }
    ...
    console.error('[Upsell] Get offers error:', error);
    res.status(500).json({
      success: false,
...
router.post('/evaluate', async (req, res) => {
  try {
    ...
    console.error('[Upsell] Evaluate triggers error:', error);
    res.status(500).json({
      success: false,
 ...
 */
router.get('/upgrade/:currentProductId/:upgradeProductId', async (req, res) => {
  try {
    ...
    console.error('[Upsell] Calculate upgrade error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to calculate upgrade value',
    });
  }
});
...
router.post('/result', async (req, res) => {
  try {
    ...
    if (!offerId || !result) {
      return res.status(400).json({
        success: false,
        error: 'offerId and result are required',
      });
    }
    ...
    console.error('[Upsell] Record result error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to record upsell result',
    });
  }
});
...
router.get('/services', async (req, res) => {
  try {
    ...
    console.error('[Upsell] Get services error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to get service recommendations',
    });
  }
});
 ...
 */
router.post('/membership-offers', async (req, res) => {
  try {
    ...
    console.error('[Upsell] Get membership offers error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to get membership offers',
    });
  }
});
 ...
 */
router.get('/financing/:amountCents', async (req, res) => {
  try {
    ...
    console.error('[Upsell] Get financing error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to get financing options',
    });
  }
});
...
router.get('/admin/analytics', async (req, res) => {
  try {
    ...
    console.error('[Upsell] Get analytics error:', error);
    res.status(500).json({
      success: false,
 ...
 */
router.get('/admin/strategies', async (req, res) => {
  try {
    ...
    console.error('[Upsell] List strategies error:', error);
    res.status(500).json({
      success: false,
 ...
 */
router.get('/admin/strategies/:id', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error('[Upsell] Get strategy error:', error);
    res.status(500).json({
      success: false,
 ...
 */
router.post('/admin/strategies', async (req, res) => {
  try {
    const {
      name,
      description,
    ...
    if (!name || !upsellType || !triggerType || !triggerValue) {
      return res.status(400).json({
        success: false,
        error: 'name, upsellType, triggerType, and triggerValue are required',
      });
    }
    ...
    console.error('[Upsell] Create strategy error:', error);
    res.status(500).json({
      success: false,
 ...
 */
router.put('/admin/strategies/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const {
      name,
    ...
    console.error('[Upsell] Update strategy error:', error);
    res.status(500).json({
      success: false,
 ...
 */
router.delete('/admin/strategies/:id', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error('[Upsell] Delete strategy error:', error);
    res.status(500).json({
      success: false,
...
router.post('/admin/offers', async (req, res) => {
  try {
    ...
    if (!strategyId || !offerTitle || !offerType || !targetType) {
      return res.status(400).json({
        success: false,
        error: 'strategyId, offerTitle, offerType, and targetType are required',
      });
    }
    ...
    console.error('[Upsell] Create offer error:', error);
    res.status(500).json({
      success: false,
...
router.put('/admin/offers/:id', async (req, res) => {
  try {
    ...
    console.error('[Upsell] Update offer error:', error);
    res.status(500).json({
      success: false,
...
router.delete('/admin/offers/:id', async (req, res) => {
  try {
    ...
    console.error('[Upsell] Delete offer error:', error);
    res.status(500).json({
      success: false,
...
router.get('/admin/services', async (req, res) => {
  try {
    ...
    console.error('[Upsell] List services error:', error);
    res.status(500).json({
      success: false,
...
router.post('/admin/services', async (req, res) => {
  try {
    ...
    if (!serviceCode || !name || !serviceType) {
      return res.status(400).json({
        success: false,
        error: 'serviceCode, name, and serviceType are required',
      });
    }
    ...
    console.error('[Upsell] Create service error:', error);
    res.status(500).json({
      success: false,
