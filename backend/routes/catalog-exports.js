...
// Run export
router.post('/:id/run', authenticate, requirePermission('catalog_exports.run'), asyncHandler(async (req, res) => {
...
router.post('/:id/download', authenticate, requirePermission('catalog_exports.view'), asyncHandler(async (req, res) => {
  const result = await catalogService.runExport(parseInt(req.params.id));
