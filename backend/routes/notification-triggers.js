...
function init({ pool }) {
  ...
  router.post('/trigger', checkPermission('manager'), asyncHandler(async (req, res) => {
    const { event, delivery_id, order_id } = req.body;
    if (!event) throw ApiError.badRequest('event is required');
    ...
    if (!validEvents.includes(event)) {
      throw ApiError.badRequest(`Invalid event. Valid: ${validEvents.join(', ')}`);
    }
    ...
    if (event === 'order.confirmed' && order_id) {
      ...
      if (!rows.length) throw ApiError.notFound('Order');
      ...
      throw ApiError.badRequest('Provide delivery_id or order_id for the event');
    }
  ...
  router.put('/config/:event_name', checkPermission('admin.settings'), asyncHandler(async (req, res) => {
    const { is_enabled } = req.body;
    if (is_enabled === undefined) throw ApiError.badRequest('is_enabled required');
    ...
    if (!rows.length) throw ApiError.notFound('Trigger config');
  ...
  router.post('/send', asyncHandler(async (req, res) => {
    ...
    if (!template_code || !customer_id) {
      throw ApiError.badRequest('template_code and customer_id are required');
    }
    ...
    if (result.queued) {
      ...
      throw ApiError.validation(result.reason || 'Failed to queue notification');
    }
  }));
  ...
  router.post('/queue/process', checkPermission('admin.settings'), asyncHandler(async (req, res) => {
    const notificationService = require('../services/NotificationTriggerService');
