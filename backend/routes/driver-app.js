...
function init({ pool }) {
  ...
  router.get('/me/shift/today', async (req, res) => {
    try {
      const driver = await getDriver(req);
      if (!driver) return res.status(404).json({ error: 'Driver not found' });
  ...
  router.post('/clock-out', async (req, res) => {
    try {
      ...
      if (!existing.rows.length) {
        return res.status(409).json({ error: 'No active shift to clock out of' });
      }
  ...
  router.post('/route/:id/start', async (req, res) => {
    try {
      const driver = await getDriver(req);
      if (!driver) return res.status(404).json({ error: 'Driver not found' });
      ...
      if (!rows.length) {
        return res.status(404).json({ error: 'Route not found or already started' });
      }
  ...
  router.post('/deliveries/:id/arrived', async (req, res) => {
    try {
      const driver = await getDriver(req);
      if (!driver) return res.status(404).json({ error: 'Driver not found' });
      ...
      if (!rows.length) {
        ...
        if (!fallback.rows.length) return res.status(404).json({ error: 'Delivery not found' });
      }
  ...
  router.post('/deliveries/:id/start', async (req, res) => {
    try {
      ...
      if (!checklist_verified) {
        return res.status(400).json({ error: 'Checklist must be verified before starting delivery' });
      }
      ...
      if (!rows.length) {
        ...
        if (!fallback.rows.length) {
          return res.status(404).json({ error: 'Delivery not found or not in arrived status' });
        }
  ...
  router.post('/deliveries/:id/send-eta', async (req, res) => {
    try {
      const driver = await getDriver(req);
      if (!driver) return res.status(404).json({ error: 'Driver not found' });
      ...
      if (!phone) return res.status(422).json({ error: 'No phone number on file' });
      ...
      console.error('Send ETA error:', err);
      res.status(500).json({ error: 'Failed to send ETA' });
    }
  });
  ...
  router.post('/issues', async (req, res) => {
    try {
      ...
      if (!category || !description || description.length < 10) {
        return res.status(400).json({ error: 'Category and description (min 10 chars) are required' });
      }
