...
router.get('/lookup', async (req, res) => {
  try {
    ...
    console.error('[POS-Quotes] Lookup error:', err);
    res.status(500).json({
      success: false,
...
router.get('/:id/for-sale', async (req, res) => {
  try {
    ...
    if (quote.status === 'converted') {
      return res.status(400).json({
        success: false,
        error: 'Quote has already been converted to a sale',
      });
    }
    ...
    console.error('[POS-Quotes] Get for sale error:', err);
    res.status(500).json({
      success: false,
      error: 'Failed to load quote for sale',
    });
  }
});
...
router.post('/:id/convert', async (req, res) => {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    console.error('[POS-Quotes] Convert error:', err);
    res.status(500).json({
      success: false,
...
router.get('/:id/status', async (req, res) => {
  try {
    ...
    console.error('[POS-Quotes] Status check error:', err);
    res.status(500).json({
      success: false,
...
router.get('/pending', async (req, res) => {
  try {
    ...
    console.error('[POS-Quotes] Pending list error:', err);
    res.status(500).json({
      success: false,
      error: 'Failed to get pending quotes',
    });
  }
});
