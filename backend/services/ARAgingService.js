  ...
  const { rows: invoices } = await pool.query(`
    WITH order_aging AS (
      SELECT
        ...
        o.total - COALESCE(pay.amount_paid, 0) AS balance_due,
      ...
      WHERE o.status NOT IN ('cancelled', 'voided', 'refunded')
        AND o.total - COALESCE(pay.amount_paid, 0) > 0
        ${locationFilter}
    )
    ...
    try {
      ...
      const { rows: [cust] } = await pool.query(
        `SELECT c.id, c.first_name, c.last_name, c.email,
                SUM(o.total - COALESCE(pay.paid, 0)) AS total_due,
         ...
         WHERE c.id = $1 AND o.total - COALESCE(pay.paid, 0) > 0
         GROUP BY c.id`,
        [custId]
      );
