...
app.post('/api/quotations/:id/send-email', async (req, res) => {
  try {
    const { id } = req.params;
    const { to_email, message, subject } = req.body;
    ...
    for (const item of itemsResult.rows) {
      itemsHTML += `
        <tr>
          ...
          <td style="padding: 8px; border: 1px solid #ddd; text-align: right;">$${((item.sell_cents || 0) / 100).toFixed(2)}</td>
          <td style="padding: 8px; border: 1px solid #ddd; text-align: right;">$${((item.line_total_cents || 0) / 100).toFixed(2)}</td>
        </tr>
      `;
    }
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="summary">
              <p><strong>Subtotal:</strong> $${((quote.subtotal_cents || 0) / 100).toFixed(2)}</p>
              ${quote.discount_cents > 0 ? `<p><strong>Discount (${quote.discount_percent}%):</strong> -$${((quote.discount_cents || 0) / 100).toFixed(2)}</p>` : ''}
              <p><strong>Tax (${quote.tax_rate}%):</strong> $${((quote.tax_cents || 0) / 100).toFixed(2)}</p>
              <p class="total"><strong>Total:</strong> $${((quote.total_cents || 0) / 100).toFixed(2)}</p>
            </div>

            ${quote.notes ? `<p><strong>Notes:</strong><br>${quote.notes.replace(/\n/g, '<br>')}</p>` : ''}
            ${quote.terms ? `<p><strong>Terms:</strong><br>${quote.terms.replace(/\n/g, '<br>')}</p>` : ''}

            <p style="margin-top: 30px; color: #6b7280; font-size: 14px;">
              Quote expires: ${new Date(quote.expires_at).toLocaleDateString()}<br>
    ...
    const emailParams = {
      ...
      Content: {
        Simple: {
          Subject: {
            Data: subject || `Quote ${quote.quote_number}`,
            Charset: 'UTF-8'
          },
          Body: {
            Html: {
    ...
    if (quote.status === 'DRAFT') {
      await pool.query(`UPDATE quotations SET status = 'SENT' WHERE id = $1`, [id]);
    }
